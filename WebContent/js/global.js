...
if (typeof (GoHomepageLink) == "undefined") {
    GoHomepageLink = "http://www.yummy77.com";
}
if (typeof (GoChiChiTuanLink) == "undefined") {
    GoChiChiTuanLink = "http://www.yummy77.com";
}
                ...
                $("#cartbarItems>div").each(function () {
                    ...
                    if (ele.attr("_id") == id) {
                        ...
                        if ($("#cartbarItems>div").length == 0) {
                            $("<div>暂时您还没有添加任何商品到购物车</div>").appendTo("#cartbarItems");
                        }
                    }
                })
            }
...
var totalprices = 0;
try {
if (item && item.length > 0 && item[0].length > 0) {
...
topHTML = " <span id=\"spanquantity\" class=\"left\" style=\"cursor:pointer;\">购物车中<b>" + quantity + "</b>件商品</span><span id=\"goClearing1\" class=\"right\" style=\"cursor:pointer;\">去结算</span>";
bottomHTML = "<div>" +
"<span class=\"left\">共<font class=\"specolor b\">" + quantity + "</font>件商品</span>" +
"<span class=\"right\">金额总计约：<font class=\"specolor b\">￥" + totalprices.toFixed(2) + "</font></span>" +
"</div>" +
"<div><span class=\"left red pointer clearCart\">清空购物车</span><span id=\"goClearing\" class=\"right specolor b balance\">去结算</span></div>";
$("#bgshoppingcart").html(topHTML);
$("#statdata").html(bottomHTML);
}
else {
returnHTML = "<div class=\"norecord\">暂无记录</div>";
topHTML = " <span id=\"spanquantity\" class=\"left\" style=\"cursor:pointer;\">购物车中<b>0</b>件商品</span><span id=\"goClearing1\" class=\"right\" style=\"cursor:pointer;\">去结算</span>";
bottomHTML = "<div>" +
"<span class=\"left\">共<font class=\"specolor b\">0</font>件商品</span>" +
...
var ShowBigImageAD = function (isManager) {
    ...
    $(document).ready(function () {
       ...
       function () { showobj.t = setInterval("showobj.showAuto();", 7000); });
    });
...
function StaticView(isShow, isShowCategories) {
    ...
    [
        ...
        {
            k: { name: "年货专区", link: "/activity/promotion/1-479.html" },
            v: ",上海,江浙站,"
        },
        {
            k: { name: "礼品区", link: "/activity/promotion/1-349.html" },
            v: ",上海,江浙站,"
        }
    ]
                    ...
                    success: function (json) {
                        ...
                        if (json.IsSuccess) {
                            ...
                            if (ddEvl.length > 0) {
                                ...
                                proNames += "</ul>";
                                jConfirm("您正在切换分站，以下商品由于暂不销售，将从购物车里移除<br/>" + proNames, null, function (result) {
                                    if (result) {
                                        $.cookie("SiteCityName", wval, { expires: 365, path: "/", domain: COOKIEDOMAIN });
                                ...
                                jConfirm("尊敬的会员，由于不同站点的商品优惠信息不同，您在切换站点时购物车内商品价格可能会有变化，敬请谅解！", null, function (result) {
                                    if (result) {
                                        $.cookie("SiteCityName", wval, { expires: 365, path: "/", domain: COOKIEDOMAIN });
                                        if (upEvl.length > 0) {
                                            SwtichSiteAction(json.upData, false);
    ...
    var LoadChannels = function () {
        ...
        channelDict.push({
            k: { name: "时令进补", link: "http://www.yummy77.com/activity/promotion/1-486.html" },
            v: ",江浙站,"
        });
        channelDict.push({
            k: { name: "时令进补", link: "http://www.yummy77.com/activity/promotion/1-486.html" },
        ...
        if (channelStr != "") {
            ...
            $("#topchannel li a").each(function () {
                ...
                if (ele.attr("href").indexOf("/activity/promotion/1-486.html") > -1) {
                    ele.after("<span class='ch_sp_tag new'></span>");
                }
                if (ele.attr("href").indexOf("/activity/promotion/1-387.html") > -1) {
                    ele.after("<span class='ch_sp_tag new2'></span>");
                }
                if (ele.attr("href").indexOf("/activity/promotion/1-479.html") > -1) {
                    ele.after("<span class='ch_sp_tag hot'></span>");
                }
...
function MenuCartView() {
    ...
    var Init = function () {
        ...
        .mouseleave(
        function () {
            clearTimeout(mouseOverWaitTime);//移除定时器
            $(".cartinfo").hide();
        });
...
function AddToCartTrackView(proListId, proListItemClassName) {
    ...
    $(".divToCartTrack").click(function () {
        ...
        setTimeout(function () { $("#smallDivAddToCart").fadeOut(); }, 3000); //间隔三秒自动关闭成功加入购物车提示框
    }).hover(function () {
        $(this).show();
    }, function () {
        $(this).hide();
...
函数说明：自定义加入购物车按钮
@params
btnClassName   加入购物车按钮的class name
proListItemClassName   列表项的 element class
...
function GenSmallAddToCartHTML() {
    var _html = "<div id=\"smallDivAddToCart\">" +
                ...
                "<div class=\"row1\">该商品已成功放入购物车中</div>" +
                "<div class=\"row2\"><span class=\"left\">共 <font class=\"specolor b\" id=\"tempQuantity\"></font> 件商品</span><span class=\"right\">约计 <font class=\"specolor b subtotal\"></font> 元</span>" +
                "</div>" +
                "<div class=\"row3\">" +
...
function ReSortDougouCards(imgType) {
    ...
    if (imgType == "small") {
        ...
        dougouLi.addClass("singleLine").prepend('<div class="clearfix"><span class="arrowOnline"></span>卡通中国</div>');
        ...
        channelUL.append('<li><span class="tips">卡通中国</span></li>');
        channelUL.append('<li class="payType singleLine"></li>');

        if (dougouChannelImages.small.length > 0) {
            dougouSpan.find("img").remove();
            $.each(dougouChannelImages.small, function () {
                dougouSpan.append('<img alt="卡通中国" title="卡通中国" src="' + this + '" height="36" width="136" />');
            })
        }
...
$(function () {
    ...
    $(".i_right .tit").mouseover(function () {
        $(".i_trends .imgli").hide();
        $(this).addClass("on").siblings().removeClass("on");
        var index = $(this).index();
...
function CategoryShow(isshow) {
    ...
    if (isshow) {
        ...
        $(".nav .firstli").hover(function () {
            $(this).find(clazz).css("display", "block");
        }, function () {
            $(this).find(clazz).css("display", "none");
        })
    }

    $(clazz).find("li").hover(function () {
...
$(function () {
    ...
    $(".r_banner li").hover(function () {
        ...
        if ($(this).hasClass("first")) {
            $(this).addClass("on").css("background-image", "url(imgs/rad_on_.gif)").siblings().not(".first").not(".last").css("background-image", "url(imgs/r_ad_noon.gif)").parent().find(".last").css("background-image", "url(imgs/r_ad_noon_1.gif)");
        } else if ($(this).hasClass("last")) {
            $(this).addClass("on").css("background-image", "url(imgs/rad_on_1.gif)").siblings().not(".first").not(".last").css("background-image", "url(imgs/r_ad_noon.gif)").parent().find(".first").css("background-image", "url(imgs/r_ad_noon_.gif)");
        } else {
            $(this).addClass("on").css("background-image", "url(imgs/rad_on.gif)").siblings().not(".first").not(".last").css("background-image", "url(imgs/r_ad_noon.gif)").parent().find(".first").css("background-image", "url(imgs/r_ad_noon_.gif)").parent().find(".last").css("background-image", "url(imgs/r_ad_noon_1.gif)");
        }
        $(img).show();
    ...
    if (browser.versions.mobile) {
        if ($.cookie("isgowap") == null) {
            if (confirm("切换到触屏版浏览更友好，是否切换？")) {
                window.location.href = "http://m.yummy77.com";
            } else {
