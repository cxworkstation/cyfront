...
var ExtTrackManager = {
    ...
    BFDTrack: {
        ...
        ProductDetail: function (_params) {
            ...
            _BFD.BFD_INFO = {
                ...
                "brand": _params.Brand, //当前商品所属品牌名称；
        ...
        ShoppingCart: function (_params) {
            ...
            _BFD.BFD_INFO = {
                "items": rItems, //[['10987',2,12.00],['10987',1,6.00],['10987',3,120.00]],// 2维数组，参数分别是['商品id号','购物车中该商品的数量','该商品的单价']
                "category": catinfo,
                "user_id": _params.UserId,
    ...
    //由集奥DSP渠道带到着陆页的到达量、购物车订单数/订单金额、提交成功订单数/订单金额
    JADSPTrack: {
        //商品详情页
        ...
        ShoppingCart: function (_params) {
            ...
            dsp_append_script(_params, ('https:' == document.location.protocol ? 'https://sslcdn.geotmt.com' : 'http://ga.istreamsche.com') + '/CPA/GEO_REGISTER.js');
    ...
    MVQTrack: {
        ...
        ProductDetail: function (_params) {
            ...
            _mvq.push(['$logConversion']);
            _mvq.push(['$addGoods', /*分类id*/_params.CategoryId, /*品牌id*/'0', /*商品名称*/_params.ProductName, /*商品ID*/_params.Id,
            /*商品售价*/_params.Price, /*商品图片url*/_params.Image, /*分类名*/_params.CategoryName, /*品牌名*/_params.BrandName,
    ...
    GoogleTrack: {
        ...
        OrderSuccess: function (_params) {
            ...
            //因为一个订单里可能包含有多种商品，每种商品的数量可以是多个，_addItem应调用购物车中的每一个商品的相关信息，需要编写一个循环应用程序loop调用购物车中的所有商品的信息，将每一个产品的信息输出为一个_addItem。
            var _items = window.eval("(" + _params.OrderItems + ")");
            $.each(_items, function (k, v) {
                _gaq.push(['_addItem',
...
function dsp_append_script(_params, _src) {
    var userid = _params;
